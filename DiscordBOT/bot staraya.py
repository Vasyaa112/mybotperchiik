...
cnx = mysql.connector.connect(user='ch93f0bb7c_discord', password='g54tg45hGTu6', host='yaris.cityhost.com.ua', database='ch93f0bb7c_discord')
...
async def balance(ctx):
    ...
    if result:
        balance = result[0]
        embed = discord.Embed(description=f"Твій баланс: {balance} Pcoin", color=0x00bfff)
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await ctx.author.send(embed=embed) # Отправляем сообщение автору команды
        embed_bot = discord.Embed(description=f"Успіx! Інформацію про баланс відправлено в особисті повідомлення користувачу {ctx.author.name}", color=0x00ff00)
        embed_bot.set_author(name="Інформація", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await channel.send(embed=embed_bot) # Отправляем сообщение в текстовый канал
    else:
        query = f"INSERT INTO users (user_id, balance) VALUES ({user_id}, 200)"
        cursor.execute(query)
        cnx.commit()
        embed = discord.Embed(description="Привіт друже! Тобі нараховано 200 Pcoin", color=0x00bfff)
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await ctx.author.send(embed=embed) # Отправляем сообщение автору команды
        embed_bot = discord.Embed(description=f"Створено нового користувача з балансом 200 Pcoin: {ctx.author.name}", color=0x00ff00)
        embed_bot.set_author(name="Інформація", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await channel.send(embed=embed_bot) # Отправляем сообщение в текстовый канал
...
async def pay(ctx, member: discord.Member = None, amount: str = None):
    ...
    if member is None or amount is None:
        embed = discord.Embed(title="Укажіть ім'я користувача і суму!", color=discord.Color.blue())
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
    ...
    if not amount.isdigit():
        ...
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        embed.add_field(name="Помилка", value="Ви можете вводити тільки прості цифри без математичних символів", inline=False)
        await ctx.send(embed=embed)
        return
    ...
    if cnx.is_connected():
        ...
        if sender_balance is None or amount > sender_balance:
            embed = discord.Embed(title="Помилка переказу", color=discord.Color.blue())
            embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
            embed.add_field(name="Помилка", value="У вас недостатньо Pcoin для виконання цієї операції", inline=False)
            await ctx.send(embed=embed)
            return
        ...
        embed = discord.Embed(title=f"Успіх! Ви успішно відправили Pcoin.", color=discord.Color.blue())
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
...
async def setpcoin(ctx, member: discord.Member = None, amount: str = None):
    ...
    if required_role1 not in ctx.author.roles and required_role2 not in ctx.author.roles:
        embed = discord.Embed(title="Помилка", description="У вас недостатньо прав використовувати цю команду!", color=discord.Color.red())
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await ctx.send(embed=embed)
        return
    ...
    if not amount.isdigit():
        embed = discord.Embed(title="Помилка", description="Сума повинна складатися тільки з цифр!", color=discord.Color.red())
        embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
        await ctx.send(embed=embed)
        return
    ...
    if cnx.is_connected():  
        ...
        if result:
            ...
            cursor.execute(query)
            cnx.commit()
            embed_bot = discord.Embed(description=f"Користувачу {member.name} було нараховано {amount} Pcoin", color=0x00ff00)
            embed_bot.set_author(name="Інформація", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
            ...
            cursor.execute(query)
            cnx.commit()
            embed_bot = discord.Embed(description=f"Створено нового користувача {member.name} з балансом {amount} Pcoin", color=0x00ff00)
            embed_bot.set_author(name="Інформація", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
            await ctx.send(embed=embed_bot) # Отправляем сообщение в текстовый канал
        cursor.close()
...
async def rempcoin(ctx, member: discord.Member = None, amount: str = None):
    ...
    if not re.match(r"^\d+$", amount):
        embed = discord.Embed(title="Невірний формат суми!", description="Укажіть коректну суму!", color=discord.Color.red())
        await ctx.send(embed=embed)
        return
    ...
    if cnx.is_connected():  
        ...
        if result:
            ...
            cursor.execute(query)
            cnx.commit()
            embed_bot = discord.Embed(description=f"У користувача {member.name} було списано {amount} Pcoin", color=discord.Color.green())
            embed_bot.set_author(name="Інформація", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
            await ctx.send(embed=embed_bot) # Отправляем сообщение в текстовый канал
        else:
            embed_bot = discord.Embed(description=f"Користувач {member.name} не знайдений у базі даних", color=discord.Color.red())
            embed_bot.set_author(name="Помилка", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760")
            await ctx.send(embed=embed_bot) # Отправляем сообщение в текстовый канал
            cursor.close()
...
async def toplist(ctx):
    ...
    if cnx.is_connected():
        ...
        if result:
            embed = discord.Embed(title="Топ(10)-багачів", description="Топ-10 користувачів серверу з найбільшою кількістю Pcoin:", color=discord.Color.blue())
            embed.set_author(name="(P)economic", icon_url="https://cdn.discordapp.com/attachments/1094564118549237760/1102885570415431742/2.png")
            for i in range(len(result)):
                user = await client.fetch_user(result[i][0])
                embed.add_field(name=f"{i+1}. {user.name}", value=f"{result[i][1]} Pcoin", inline=False)
            await ctx.send(embed=embed)
        else:
